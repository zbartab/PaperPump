...
Then we plot the graph. Here the width of the edges is proportional to $p_{A,B}$, i.e. the proportion of shared articles. The size of the nodes is proportional the number of D1 articles published by the given person (node).
...
guild.productivity <- function(guild, cikkek=papers.ls, szerzok=ee.prod) {
	...
	d <- data.frame(n=unlist(sapply(p, names)), v=unlist(p))
	ud <- unique(d)
	r <- numeric()
	r["g.papers"] <- nrow(ud)
...
guild.productivity.old <- function(guild, cikkek=papers.ls, szerzok=ee.prod) {
...
rnd.guild.productivity <- function(guild.size, n.rep=1000, cikkek=papers.ls,
	...
	res <- matrix(0, ncol=14, nrow=n.rep)
	for(i in 1:n.rep) {
...
if(file.exists(gd.file)) {
	...
	save(cl.df, file="guild_measures.Rdata")
}
```
...
library(lme4)
library(emmeans)
cits <- grepl("citations$", cl.df$variable)
m.papers <- lmer(log10(g2rnd) ~ variable + (1|guild), cl.df, subset=!cits)
drop1(m.papers, test="Chisq")
summary(m.papers)
(em.p <- emmeans(m.papers, ~variable, type="response"))
pairs(em.p)
...
m.citations <- lmer(log10(g2rnd+1) ~ variable + (1|guild), cl.df, subset=cits)
drop1(m.citations, test="Chisq")
summary(m.citations)
(em.c <- emmeans(m.citations, ~variable, type="response"))
pairs(em.c)
